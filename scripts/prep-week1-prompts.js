...
const schedule = JSON.parse(fs.readFileSync(path.join(repoRoot, "Data/2026/schedule/week-01.json"), "utf8"));
const promoted = new Set(NFL_GAMES.map(g => g.id));
const remaining = schedule.games.filter(g => !promoted.has(g.id));

function slugify(name) {
  return name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
}

function kickoffDisplay(g) {
...
function commonBody(g, opts) {
  ...
  const kickoff = kickoffDisplay(g);
  const network = g.network;
  const venue = g.neutral_site || g.venue;
  const gameId = g.id;

  return `You are an independent entry in the CSolutions AI Analyzer NFL
study, Week 1, ${g.away} at ${g.home}. Kickoff ${kickoff} at ${venue},
broadcast on ${network}. game_id: ${gameId}.
...
function localProfileCitation(awaySlug, homeSlug) {
  return `- Data/2026/rosters/${awaySlug}.json
- Data/2026/rosters/${homeSlug}.json`;
}
...
const GITHUB_EVIDENCE = `You do not have local filesystem access. The study's tracked graded data
...
`;
  ...
  fs.writeFileSync(path.join(folder, "prompt-local.md"), localHeader);
  fs.writeFileSync(path.join(folder, "prompt-github.md"), githubHeader);
