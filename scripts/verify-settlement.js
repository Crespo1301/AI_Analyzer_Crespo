...
const tickets = game.humanComparison.tickets;
assert.equal(Math.round(tickets.reduce((sum, t) => sum + t.stake, 0) * 100), 2287);
assert.equal(Math.round(tickets.reduce((sum, t) => sum + t.returned, 0) * 100), 4197);
assert.equal(Math.round(tickets.reduce((sum, t) => sum + t.returned - t.stake, 0) * 100), 1910);
assert.match(tickets[2].note, /voided/);
